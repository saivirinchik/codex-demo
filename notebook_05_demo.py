# =============================================================================
# CDOCS POC — Notebook 05: Live Classification Demo
# =============================================================================
#
# PURPOSE:
#   Interactive demo for stakeholders. Upload a new PDF to DBFS,
#   enter the filename, and the notebook runs the full pipeline:
#
#     PDF → text extraction → quality check → LLM summary → ML prediction
#
#   Shows the predicted classification, confidence, alternatives,
#   and the LLM summary — all in the notebook.
#
# DEPENDS ON:
#   - Trained model saved to DBFS (from Notebook 04 Cell 15)
#   - AI Gateway credentials (same widgets as Notebook 02)
#
# SETUP:
#   1. Upload test PDF(s) to dbfs:/FileStore/cdocs_poc/demo_docs/
#   2. Fill in the AI Gateway widgets
#   3. Run all cells top to bottom
#   4. Enter the filename in the DEMO_FILENAME widget
#   5. Run the demo cell
#
# =============================================================================
...
MODEL_PATH = "/dbfs/FileStore/cdocs_poc/model/cdocs_classifier.joblib"

print("Widgets ready — fill in values at the top of the notebook")


# -----------------------------------------------------------------------------
# CELL 4 — Load trained model
# -----------------------------------------------------------------------------
assert os.path.exists(MODEL_PATH), f"Model not found: {MODEL_PATH}. Run Notebook 04 Cell 15 first."

pipeline = joblib.load(MODEL_PATH)
class_labels = list(pipeline.classes_)

print(f"Model loaded from: {MODEL_PATH}")
print(f"Classification categories ({len(class_labels)}):")
...
# Run this to see which files are available for classification.
# -----------------------------------------------------------------------------
...
print(f"Demo documents in {DEMO_DIR}:")
if demo_files:
    for f in demo_files:
        size = os.path.getsize(os.path.join(DEMO_DIR, f))
        print(f"  📄 {f}  ({size:,} bytes)")
else:
    print("  (none found — upload PDFs to dbfs:/FileStore/cdocs_poc/demo_docs/)")


# -----------------------------------------------------------------------------
# CELL 6 — Pipeline functions (same as Notebooks 01 & 02)
# All functions defined in a single cell so nothing breaks after restart.
# -----------------------------------------------------------------------------
...
def score_text_quality(text):
    ...
    word_ratio = sum(1 for w in words if sum(c.isalpha() for c in w) >= 2) / len(words) if words else 0
    special = sum(1 for c in text if not c.isalnum() and c not in ' \t\n\r.,;:!?\'"()-/\\@#$%&*+=[]{}|<>~`_')
    special_ratio = special / total
    garbled = len(re.findall(r'[^\w\s]{10,}', text))
    score = 0.40*alpha_ratio + 0.25*(1-min(special_ratio*10,1)) + 0.25*word_ratio + 0.10*(1-min(garbled/20,1))
...
3. Do NOT mention or guess departments, business units, or organizational categories.
...
#
# Set the DEMO_FILENAME widget at the top, then run this cell.
# This is the cell you re-run during the live demo for each document.
# -----------------------------------------------------------------------------
...
if not demo_filename:
    ...
    if extraction["score"] < 0.40:
        print(f"\n❌ Text quality too low ({extraction['score']:.2f}). Route to HUMAN REVIEW.")
    else:
        ...
        print(f"\n🎯 Step 3: Predicting classification...")
        ...
        print(f"\n   {'Rank':<6} {'Classification':<40} {'Probability':>12}")
        print(f"   {'─'*6} {'─'*40} {'─'*12}")

        for rank, i in enumerate(sorted_idx[:5], 1):
            marker = " ◀" if rank == 1 else ""
            print(f"   {rank:<6} {class_labels[i]:<40} {probabilities[i]:>11.1%}{marker}")
        ...
        if llm_conf == "low" or top_prob < 0.40:
            ...
            print(f"   Reason: {'LLM confidence low' if llm_conf == 'low' else 'Prediction confidence low'}")
            print(f"{'='*70}")


# -----------------------------------------------------------------------------
# CELL 8 — Rich HTML display (optional — more visual for stakeholders)
# Run this cell AFTER Cell 7 for a formatted output.
# Only works if Cell 7 ran successfully on a document.
# -----------------------------------------------------------------------------

try:
    _ = prediction  # check if Cell 7 ran
    ...
    for rank, i in enumerate(sorted_idx[:5], 1):
        ...
        <div style="margin-bottom: 8px;">
            <div style="display: flex; justify-content: space-between; font-size: 14px;">
                <span>{'→ ' if rank == 1 else '&nbsp;&nbsp;'}<b>{class_labels[i]}</b></span>
                <span>{pct:.1f}%</span>
            </div>
    ...
    if llm_conf == "low" or top_prob < 0.40:
        ...
        <div style="background: #fef2f2; border: 1px solid #fca5a5; border-radius: 8px;
            ...
            <b>⚠ Recommendation:</b> Route to human review
        </div>"""
    ...
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;
        ...
        <h2 style="margin-bottom: 4px;">📄 Classification Result</h2>
        ...
        <div style="display: flex; gap: 16px; margin: 20px 0;">
            <div style="flex: 2; background: #f8fafc; border: 1px solid #e2e8f0;
                ...
                <div style="color: #64748b; font-size: 13px; text-transform: uppercase;
                            letter-spacing: 0.5px;">Predicted Classification</div>
                <div style="font-size: 24px; font-weight: 700; margin-top: 8px;">{prediction}</div>
            </div>
        ...
        <details style="margin-top: 24px;">
            ...
            <p style="color: #64748b; font-size: 13px;"><b>Key terms:</b> {', '.join(key_terms[:15])}</p>
        </details>
        ...
        <details style="margin-top: 12px;">
            ...
            <table style="margin-top: 8px; font-size: 13px; color: #475569;">
                <tr><td style="padding: 2px 12px 2px 0;"><b>Method</b></td><td>{extraction['method']}</td></tr>
                <tr><td style="padding: 2px 12px 2px 0;"><b>Pages</b></td><td>{extraction['pages']}</td></tr>
                <tr><td style="padding: 2px 12px 2px 0;"><b>Characters</b></td><td>{len(extraction['text']):,}</td></tr>
                <tr><td style="padding: 2px 12px 2px 0;"><b>Text quality</b></td><td>{extraction['score']:.3f}</td></tr>
                <tr><td style="padding: 2px 12px 2px 0;"><b>LLM confidence</b></td><td>{llm_conf}</td></tr>
            </table>
        </details>
    </div>
    """
